...
int ctrInput, // Valor lido no potenciômetro 
    ...
    dtaBathTime = 120; // Tempo de banho, por enquanto mantido estático, será atualizado via app
...
void show(int number) {
  ...
  for(int i : ctrDisplay[number]) { 
    ...
    digitalWrite(DSP_CLK, LOW); // Pulso positivo de clock
    digitalWrite(DSP_CLK, HIGH); // Pulso negativo de clock
...
void buzzer(){
  Serial.println("buzzer");
    ...
    if(_now - ctrStart >= (dtaBathTime - 30)* 1000 && dtaActive) {
